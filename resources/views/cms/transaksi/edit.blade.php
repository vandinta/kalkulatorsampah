            ...
            <div class="col-md-6 col-lg-12">
              <form action="{{ route('transaksi.update', $transaksi->id) }}" method="post">
                ...
                <div class="form-group">
                  ...
                  <input type="text" class="form-control" id="nama" name="nama" placeholder="Nama User" value="{{ $transaksi->nama }}" autofocus>
                </div>
                <div class="form-group">
                  ...
                  <select class="form-control" id="id_sampah" name="id_sampah" onchange="Hitung(this);" autofocus>
                    ...
                    <option data-harga="<?= $sp['harga'] ?>" data-nama="<?= $sp['nama'] ?>" value="<?= $sp['id'] ?>" {{ $transaksi->id_sampah == $sp['id'] ? 'selected' : '' }}><?= $sp['nama'] ?></option>
                    @endforeach
                  </select>
                </div>
                <div class="form-group">
                  <label for="berat">Berat Sampah</label>
                  <input type="number" class="form-control" id="berat" name="berat" placeholder="Berat Sampah" value="{{ $transaksi->berat }}" onchange="Hitung(this);" autofocus>
                </div>
                <div class="form-group">
                  <label for="harga">Harga Total</label>
                  <input type="number" class="form-control" id="harga" name="harga[]" placeholder="Harga Total" value="{{ $transaksi->harga }}" disabled>
                </div>
                <input type="hidden" id="harga_hide" name="harga_hide[]" value="{{ $transaksi->harga }}">
                <div class="form-group">
                  ...
                  <select class="form-control" id="status" name="status" autofocus>
                    ...
                    <option value="panding" {{ $transaksi->status == 'panding' ? 'selected' : '' }}>Panding</option>
                    <option value="cancel" {{ $transaksi->status == 'cancel' ? 'selected' : '' }}>Cancel</option>
                  </select>
                </div>
